...
translate_op = {"A":"X","B":"Y","C":"Z"}
dico_points = {"X" : 1, "Y": 2, "Z": 3}
...
with open("input", "r") as f:
    ...
    score_total = 0
    for l in lines:
        opponent = translate_op[l[0]]
        me = l[2]
        if me == opponent:
            score_manche = D + dico_points[me]
        elif (me == "X" and opponent == "Z") or (me == "Y" and opponent == "X") or (me == "Z" and opponent == "Y"):
            score_manche = W + dico_points[me]
        else: score_manche = dico_points[me]
...
with open('input', 'r') as f:
    ...
    for l in lines:
        ...
        if end == "X": # Défaite
            score_manche = ndp[cgp[move]]
        elif end == "Y": # Draw
            score_manche = ndp[move] + D
        else: score_manche = ndp[[key for key, value in cgp.items() if value == move][0]] + W
