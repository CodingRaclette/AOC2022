...
with open("input", "r") as f:
    ...
    compteur = 0
    for l in lines:
        elfe1 = l.split(",")[0].split("-")
        elfe2 = l.split(",")[1].split("-")
        if int(elfe1[0]) <= int(elfe2[0]) and int(elfe1[1]) >= int(elfe2[1]):
            compteur += 1
            continue
        elif int(elfe2[0]) <= int(elfe1[0]) and int(elfe2[1]) >= int(elfe1[1]):
            compteur += 1
            continue
        l_elfe2 = range(int(elfe2[0]), int(elfe2[1])+1)
        print(l_elfe2[-1], int(elfe2[0]), int(elfe2[1]))
        if int(elfe1[0]) in l_elfe2 or int(elfe1[1]) in l_elfe2:
            compteur+=1
            continue
